...
api_response = requests.post('https://replyiq.com/api/ai/generate',
  headers=headers,
...
response = requests.post('https://replyiq.com/api/ai/generate',
    headers=headers,
    json={
        'text': 'Original post content',
        'tone': 'professional',
...
```curl cURL
curl -X POST https://replyiq.com/api/ai/generate \
